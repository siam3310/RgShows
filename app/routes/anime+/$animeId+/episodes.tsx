...
export const meta = mergeMeta<{}, { 'routes/anime+/$animeId': typeof animeIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `RG SHOWS - ${animeTitle} - Episodes` },
      {
        name: 'description',
        content: description
          ? description?.replace(/<\/?[^>]+(>|$)/g, '')
          : `Watch ${animeTitle} in RG SHOWS`,
      },
      {
        property: 'og:url',
        content: `https://rgshows.vercel.app/anime/${params.animeId}/episodes`,
      },
      { property: 'og:title', content: `RG SHOWS - ${animeTitle} - Episodes` },
      {
      ...
      { name: 'twitter:title', content: `RG SHOWS - ${animeTitle} - Episodes` },
      {
