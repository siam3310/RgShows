...
export const meta = mergeMeta<typeof loader>(({ data, params }) => {
  ...
  const { detail } = data;
  return [
    { title: `RG SHOWS - ${detail?.name || ''}` },
    { name: 'description', content: `${detail?.description || ''}` },
    { name: 'keywords', content: `${detail?.name || ''}` },
    {
      property: 'og:url',
      content: `https://rgshows.vercel.app/lists/${params.listId}`,
    },
    { property: 'og:title', content: `RG SHOWS - ${detail?.name || ''}` },
    { property: 'og:description', content: `${detail?.description || ''}` },
    { name: 'twitter:title', content: `RG SHOWS - ${detail?.name || ''}` },
