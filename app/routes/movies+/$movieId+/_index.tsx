...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ matches, params }) => {
    ...
    const { title } = detail || {};
    return [
      { title: `RG SHOWS - ${title}` },
      { property: 'og:title', content: `RG SHOWS - ${title}` },
      { property: 'og:url', content: `https://rgshows.vercel.app/movies/${params.movieId}/` },
      { property: 'twitter:title', content: `RG SHOWS - ${title}` },
    ];
  },
);
