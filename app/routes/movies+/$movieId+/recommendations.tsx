...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `RG SHOWS - ${movieTitle} - Recommendations` },
      {
        property: 'og:url',
        content: `https://rgshows.vercel.app/movies/${params.movieId}/recommendations`,
      },
      { property: 'og:title', content: `RG SHOWS - ${movieTitle} - Recommendations` },
      { name: 'twitter:title', content: `RG SHOWS - ${movieTitle} - Recommendations` },
    ];
  },
);
